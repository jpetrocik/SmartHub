		...
		request('https://api.sunrise-sunset.org/json?lat=' + lat + '&lng=' + lon + '&date=' + when + '&formatted=0', { json: true }, (err, res, body) => {
			...
			if ( sunrise > SunsetModule.SUNRISE_TIME) {
				...
				if (sunriseTriggerDelay > 0) {
					setTimeout(() => {
						console.log("Firing sunrise event");
						SmartHub.emit('SunsetModule.SUNRISE')
					}, sunriseTriggerDelay);
				}
			}
			...
			if ( sunset > SunsetModule.SUNSET_TIME) {
				...
				if (sunsetTriggerDelay > 0) {
					setTimeout(() => {
						console.log("Firing sunset event");
						SmartHub.emit('SunsetModule.SUNSET')
					}, sunsetTriggerDelay);
				}
			}

			setTimeout(() => {requestSunset(SmartHub)}, refreshDelay);

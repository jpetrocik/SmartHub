	...
	silence(topic, hash) {
		this.config.conditions[topic].forEach((condition) => {
			if ( condition.hash == hash){
				...
				setTimeout(this.clearSilence, 60*1000, condition);
			}
		})
	}
	...
	processMessage(topic, condition, msg) {
		...
		if (expressionResult) {
			...
			if (!condition.suppress) {
				...
				if (condition.sms) {
					let smsMessage = condition.message + "\n\n\nTo silence for 24 hours\nhttp://192.168.1.88:2157/api/notification/silence/" + topic + "?hash=" + condition.hash;
					condition.sms.forEach((phoneNumber) => {
						this.emit('SmsModule.SEND', phoneNumber, smsMessage);
					});
				}
			}
		}
...
NotificationModule.init = function(SmartHub){
	let notificationModule = new NotificationModule(SmartHub);
